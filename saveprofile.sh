...
#
	...
	##	sudo sed -i '/^\[/h;G;/Profile '"$2"'/s/\(Password=\).*/\1'"$6"'/m;P;d'  /usr/local/etc/Nextion_Support/profiles.txt
		sudo sed -i '/^\[/h;G;/Profile '"$2"'/s/\(Name=\).*/\1'"$7"'/m;P;d'  /usr/local/etc/Nextion_Support/profiles.txt
	...
	##	sudo sed -i '/^\[/h;G;/Profile 0/s/\(Password=\).*/\1'"$6"'/m;P;d'  /usr/local/etc/Nextion_Support/profiles.txt
		sudo sed -i '/^\[/h;G;/Profile 0/s/\(Name=\).*/\1'"$7"'/m;P;d'  /usr/local/etc/Nextion_Support/profiles.txt
		...
		sudo sed -i '/^\[/h;G;/Networks/s/\(TGIF=\).*/\1'"$6"'/m;P;d'  /home/pi-star/psfile.txt
        fi

        if [ "$pw1" == mnet ]; then
		sudo sed -i '/^\[/h;G;/Networks/s/\(MNET=\).*/\1'"$6"'/m;P;d'  /home/pi-star/psfile.txt
        fi
        if [ "$pw1" == branmeister ]; then
		sudo sed -i '/^\[/h;G;/Networks/s/\(BM=\).*/\1'"$6"'/m;P;d'  /home/pi-star/psfile.txt
        fi
