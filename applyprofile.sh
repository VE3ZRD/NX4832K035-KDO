...
dirn="/usr/local/etx/Nextion_Support/profiles.txt
...
sudo sed -i '/^\[/h;G;/Profile 0/s/\(RXFrequency=\).*/\1'"$3"'/m;P;d'  /usr/local/etc/Nextion_Support/profiles.txt
sudo sed -i '/^\[/h;G;/Profile 0/s/\(TXFrequency=\).*/\1'"$4"'/m;P;d'  /usr/local/etc/Nextion_Support/profiles.txt
...
sudo sed -i '/^\[/h;G;/Profile 0/s/\(TalkGroup=\).*/\1'"$m11"'/m;P;d'  /usr/local/etc/Nextion_Support/profiles.txt
...
if [ "$8" == "DMRG" ]; then
	sudo /usr/local/etc/Nextion_Support/setysf2dmr.sh 127.0.0.1
fi
if [ "$8" == "YSF" ]; then
