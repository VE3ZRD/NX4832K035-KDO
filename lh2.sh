...
if [ "$2" == "1" ]; then
	...
	if [ "$1" == "1" ]; then
		echo "$list3" | sed -n '1,7p;8q' |  awk '{printf "%5s %5s %7s %6s %1s %4s %-4s|\n", $1, $2, $3, $4, $5, $6, $7}' | tr -d "\n"
	fi
	if [ "$1" == "2" ]; then
		echo "$list3" | sed -n '7,12p;13q' |  awk '{printf "%5s %5s %6s %6s %1s %4s %-4s|\n", $1, $2, $3, $4, $5, $6, $7}' | tr -d "\n"
	fi
	if [ "$1" == "3" ]; then
...
######  Mode 2/3/4
	...
	if [ "$1" == "1" ]; then
		list9=$(echo "$list3" | sed -n '1,8p;9q' | awk '{print $1, $2, $3, $4}' )
	fi
	if [ "$1" == "2" ]; then
